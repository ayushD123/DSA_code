...
nd* create(){
    ...
    printf("Enter SSN,Phone Number,Name,Dept,Desg,SAlary: ");
    scanf("%d %s %s %s %s %f",&(new->ssn),new->phno,new->name,new->dept,new->desg,&(new->sal));
    return new;
}
...
void display(nd* root){
    ...
    if(p==NULL){
        ...
        while(p!=NULL){
        printf("%d %s %s %s %s %f \n",(p->ssn),p->phno,p->name,p->dept,p->desg,(p->sal));
        p=(p->right);
    }
    printf("\n");
    }
...
nd* delete_f(nd* root){
    ...
    printf("%d %s %s %s %s %f\n",(root->ssn),root->phno,root->name,root->dept,root->desg,(root->sal));
    root=temp->right;
    temp->right=NULL;
    root->left=NULL;
    return root;
